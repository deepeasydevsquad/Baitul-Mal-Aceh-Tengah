...
class Model_r {
  ...
  async daftar_syarat() {
    ...
    if (body.search && body.search !== "") {
      where.name = { [Op.like]: `%${body.search}%` }; // ✅ sesuai field DB
    }
    ...
    const sql = {
      ...
      offset: (page - 1) * limit,
      order: [["id", "ASC"]],
      attributes: ["id", "name", "path", "createdAt", "updatedAt"], // ✅ sesuai DB
      where,
    };
  ...
  async detail_syarat() {
    const { id } = this.req.params; // ambil id dari params (GET /syarat/:id)
