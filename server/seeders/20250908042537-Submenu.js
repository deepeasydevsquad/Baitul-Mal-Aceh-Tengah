...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    await queryInterface.bulkInsert('Submenus', [
      ...
      { menu_id : menus[4].id, name: 'Laporan Kesekretariatan', path: 'laporan_kesekretariatan', tab:`[{"id":"${tabs[46].id}"}]`,  createdAt: new Date(), updatedAt: new Date() },
      { menu_id : menus[4].id, name: 'Rekap', path: 'rekap', tab:`[{"id":"${tabs[47].id}"},{"id":"${tabs[48].id}"},{"id":"${tabs[49].id}"},{"id":"${tabs[50].id}"},{"id":"${tabs[51].id}"},{"id":"${tabs[52].id}"}]`,  createdAt: new Date(), updatedAt: new Date() },
