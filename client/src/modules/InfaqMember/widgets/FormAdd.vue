...
const handleConfirmPayment = async () => {
  ...
  try {
    ...
    emit('status', {
      error: false,
      error_msg:
        'Pembayaran infaq berhasil dicatat, Silahkan konfirmasi pembayaran dengan menekan tombol kuning.',
    });
    emit('close');
  } catch (error: any) {
        ...
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-gray-800">
              <span v-if="currentStep === 1">Pembayaran Infaq</span>
          ...
          <div v-if="currentStep === 1" class="space-y-6">
            <div class="text-center bg-gray-50 p-4 rounded-lg">
              <p class="text-sm text-gray-600 italic">
                "Berimanlah kamu kepada Allah dan Rasul-Nya dan infakkanlah sebagian dari hartamu
                yang Allah telah menjadikan kamu menguasainya. Maka orang-orang yang beriman di
                antara kamu dan menginfakkan (sebagian) dari hartanya memperoleh pahala yang besar."
              </p>
              <p class="text-xs font-semibold text-gray-700 mt-2">(QS Al-Hadid: 7)</p>
            </div>
            <div>
              <label for="jumlahInfaq" class="block text-sm font-medium text-gray-700 mb-1"
                >Jumlah Infaq Anda</label
              >
              ...
              <p class="mt-1 text-xs text-gray-500">
                Silahkan masukkan jumlah infaq yang ingin anda bayar disini.
              </p>
            </div>
            <BaseButton @click="nextStep" variant="primary" class="w-full"
              >Bayar Infaq Sekarang</BaseButton
            >
          </div>
          ...
          <div v-if="currentStep === 2" class="space-y-4">
            ...
            <div v-else class="space-y-4">
              <div class="bg-gray-50 p-3 rounded-lg">
                <label class="block text-xs font-medium text-gray-500">Nominal Infaq</label>
