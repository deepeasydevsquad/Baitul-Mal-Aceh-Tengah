...
const data_pertanyaan_monitoring = ref<PertanyaanMonitoring[]>([]);
const jawaban = ref<Record<number, string>>({});

const fetch_pertanyaan = async () => {
  try {
    const response = await pertanyaan_monitoring();
    console.log('Response pertanyaan:', response);
    data_pertanyaan_monitoring.value = response.data;

    // Inisialisasi jawaban kosong
    response.data.forEach((item: PertanyaanMonitoring) => {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log('Jawaban monitoring terkirim:', response);

    // 🔹 Emit ke parent agar status berubah otomatis
    emit('status', { error: false, error_msg: '' });

    // 🔹 Notifikasi sukses
    displayNotification('Jawaban monitoring berhasil dikirim!', 'success');
  } catch (error: any) {
    ...
    >
      ...
      >
        ...
        <div class="flex items-center justify-between">
          <h2 id="modal-title" class="text-xl font-bold text-gray-800">Monitoring Monev</h2>
          <button
        ...
        <div class="space-y-4 border-t border-gray-200 pt-4">
          <h3 class="font-semibold text-gray-700">Pertanyaan Monitoring</h3>

          <div
            v-for="item in data_pertanyaan_monitoring"
            :key="item.id"
