...
async function fetchData() {
  ...
  try {
    const response = await get_info_pemberitahuan(
    ...
    console.log('daftar berkas rejected', dataInfo.value);
  } catch (error: any) {
...
const rejectedBerkas = computed(() => {
  return dataInfo.value?.berkas_rejected || [];
});

// Computed: WhatsApp message preview
const whatsappMessage = computed(() => {
  return dataInfo.value?.whatsapp_message || '';
});
  ...
  >
    ...
    >
      <div class="relative max-w-2xl w-full bg-white shadow-2xl rounded-2xl p-6 space-y-6">
        ...
        <div class="space-y-4 max-h-[64vh] overflow-y-auto no-scrollbar">
          <!-- Info Pemohon -->
          ...
          <div
            v-if="!dataInfo?.is_processing"
            class="bg-gray-50 border border-gray-300 rounded-xl p-4"
          >
          ...
          >
            ...
            <p class="text-xs text-yellow-800">
              Saat ini, validasi berkas sedang dalam proses, dan tidak ada berkas yang ditolak.
            </p>
          </div>

          <!-- Warning Box -->
          <div
            v-if="rejectedBerkas.length > 0 && !dataInfo?.is_processing"
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="text-xs text-yellow-800">
                ...
                <p>
                  Pesan WhatsApp akan dikirim ke pemohon berisi informasi berkas yang ditolak
                  beserta instruksi untuk melengkapi berkas.
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="flex justify-end gap-3">
          ...
          <BaseButton
            type="button"
            variant="success"
            :disabled="rejectedBerkas.length === 0 || isSubmitting || dataInfo?.is_processing"
            @click="handleSubmit"
          >
