...
// Service API
import { get_laporan_kesekretariatan, get_tahun, download_excel_kesekretariatan } from '@/service/laporan_kesekretariatan';
...
async function fetchData() {
  isTableLoading.value = true;
  try {
    const response = await get_laporan_kesekretariatan(selectedTahun.value);
...
async function downloadExcel() {
  try {
    const response = await download_excel_kesekretariatan(selectedTahun.value, 'amil');
    ...
    link.href = url;
    const tahunText = selectedTahun.value === '0' ? 'SemuaTahun' : selectedTahun.value;
    link.download = `Laporan_Kesekretariatan_${tahunText}.xlsx`;
    ...
    <div v-else class="space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <BaseButton @click="downloadExcel" variant="primary" type="button">
          ...
          Download Excel Laporan Kesekretariatan
        </BaseButton>
        <select
