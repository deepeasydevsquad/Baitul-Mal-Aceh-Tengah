  ...
  try {
    ...
    rataPersentase.value =
      validPercentages.length > 0
        ? validPercentages.reduce((acc, d) => acc + parseFloat(d.persentaseTotal), 0) / validPercentages.length
...
function setupCharts() {
  ...
  series.value = [
    { name: 'Zakat', data: zakatData ? getMonthlyDistribution(zakatData.realisasi) : Array(12).fill(0) },
    { name: 'Infaq', data: infaqData ? getMonthlyDistribution(infaqData.realisasi) : Array(12).fill(0) },
    { name: 'Donasi', data: donasiData ? getMonthlyDistribution(donasiData.realisasi) : Array(12).fill(0) },
  ]
    ...
    if (!jenisData) {
      return {
        ...
        options: {
          ...
          title: { text: title, align: 'left', style: { color: '#374151', fontSize: '13px', fontWeight: '600' } },
...
<template>
  <div class="mx-auto p-4 bg-gray-50">
    ...
    <div v-else class="space-y-4">
      ...
      <div class="grid grid-cols-4 gap-4">
        <div class="p-4 rounded-lg bg-white text-center shadow-sm border border-gray-200">
          ...
          <p class="text-2xl font-bold text-gray-900">{{ formatRupiah(selectedYearData?.totalRealisasi || 0) }}</p>
        </div>
        <div class="p-4 rounded-lg bg-white text-center shadow-sm border border-gray-200">
          <h3 class="text-sm font-medium text-gray-600 mb-1">Target</h3>
          <p class="text-2xl font-bold text-gray-900">{{ formatRupiah(selectedYearData?.totalTarget || 0) }}</p>
        </div>
        <div class="p-4 rounded-lg bg-white text-center shadow-sm border border-gray-200">
        ...
        <div class="p-4 rounded-lg bg-white text-center shadow-sm border border-gray-200">
          ...
          <p class="text-2xl font-bold text-gray-900">{{ selectedYearData?.persentaseTotal || '0.00' }}%</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="p-4 rounded-lg border shadow-sm bg-white">
          <VueApexCharts v-if="zakatChart.series && zakatChart.series.length > 0" :options="zakatChart.options" :series="zakatChart.series" height="280" />
          <div v-else class="flex items-center justify-center h-[280px] text-gray-400">Tidak ada data</div>
        </div>
        
        <div class="p-4 rounded-lg border shadow-sm bg-white">
          <h3 class="text-sm font-semibold text-gray-700 mb-3">Pengumpulan per Bulan</h3>
          <VueApexCharts v-if="series.length > 0" type="bar" height="280" :options="chartOptions" :series="series" />
          <div v-else class="flex items-center justify-center h-[280px] text-gray-400">Tidak ada data</div>
        </div>

        <div class="p-4 rounded-lg border shadow-sm bg-white">
          <VueApexCharts v-if="infakChart.series && infakChart.series.length > 0" :options="infakChart.options" :series="infakChart.series" height="280" />
          <div v-else class="flex items-center justify-center h-[280px] text-gray-400">Tidak ada data</div>
        </div>
        ...
        <div class="p-4 rounded-lg border shadow-sm bg-white">
          ...
          <div v-if="selectedYearData && selectedYearData.dataPerJenis && selectedYearData.dataPerJenis.length > 0">
            <table class="w-full text-xs">
              ...
              <tbody class="divide-y">
                <tr v-for="jenis in selectedYearData.dataPerJenis" :key="jenis.jenis" class="hover:bg-gray-50">
                ...
                <tr class="bg-gray-50 font-semibold">
                  ...
                  <td class="py-2 px-2 text-right">{{ formatRupiah(selectedYearData.totalTarget) }}</td>
                  <td class="py-2 px-2 text-right">{{ formatRupiah(selectedYearData.totalRealisasi) }}</td>
                  <td class="py-2 px-2 text-right">{{ (selectedYearData.persentaseTotal ?? 0).toFixed(2) }}%</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div v-else class="flex items-center justify-center h-32 text-gray-400">Tidak ada data</div>
        </div>

        <div class="p-4 rounded-lg border shadow-sm bg-white">
          <VueApexCharts v-if="donasiChart.series && donasiChart.series.length > 0" :options="donasiChart.options" :series="donasiChart.series" height="280" />
          <div v-else class="flex items-center justify-center h-[280px] text-gray-400">Tidak ada data</div>
        </div>
                ...
                <tr v-for="jenis in selectedYearData.dataPerJenis" :key="jenis.jenis" class="hover:bg-gray-50">
                  ...
                  <td class="py-2 px-2 text-right">
                    <div class="flex items-center justify-end gap-1">
                      <div class="h-4 bg-blue-400 rounded" :style="{ width: `${Math.min(parseFloat(String(jenis.persentase))  * 0.6, 100)}%`, maxWidth: '100px' }"></div>
                      <span class="text-xs whitespace-nowrap">{{ formatRupiah(jenis.realisasi * 0.6) }}</span>
                    </div>
                  </td>
                  <td class="py-2 px-2 text-right">
                    <div class="flex items-center justify-end gap-1">
                      <div class="h-4 bg-cyan-400 rounded" :style="{ width: `${Math.min(parseFloat(String(jenis.persentase)) * 0.8, 100)}%`, maxWidth: '100px' }"></div>
                      <span class="text-xs whitespace-nowrap">{{ formatRupiah(jenis.realisasi * 0.4) }}</span>
                    </div>
                  </td>
                </tr>
                <tr class="bg-gray-50 font-semibold">
                  <td class="py-2 px-2">Total keseluruhan</td>
                  <td class="py-2 px-2 text-right">{{ formatRupiah(selectedYearData.totalRealisasi * 0.6) }}</td>
                  <td class="py-2 px-2 text-right">{{ formatRupiah(selectedYearData.totalRealisasi * 0.4) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
