...
import { useSidebarStore, useSelectedTab, useGlobalTab, useGlobalActiveTab, useTabTerpilih, globalSelectMenu } from '../../../../stores/sidebar'
...
watch( () => props.menu_info, (newVal) => { if (newVal) { dataRef.value = newVal; } },
  { immediate: true },
)
...
<template>
  <aside
    class="absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-white duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 shadow-lg">
    <div class="flex items-center justify-center gap-2 px-6 py-2.5 lg:py-3.5">
      <router-link to="/">
        <img :src="'/images/logo.png'" alt="Logo" class="h-14" />
        <!-- <div class="relative w-60 h-17 rounded-t-lg ">
      ...
      <button class="block lg:hidden" >
        <svg class="fill-current" width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill="#000000" fill-rule="evenodd" d="M19 8.175H2.98748L9.36248 1.6875C9.69998 1.35 9.69998 0.825 9.36248 0.4875C9.02498 0.15 8.49998 0.15 8.16248 0.4875L0.399976 8.3625C0.0624756 8.7 0.0624756 9.225 0.399976 9.5625L8.16248 17.4375C8.31248 17.5875 8.53748 17.7 8.76248 17.7C8.98748 17.7 9.17498 17.625 9.36248 17.475C9.69998 17.1375 9.69998 16.6125 9.36248 16.275L3.02498 9.8625H19C19.45 9.8625 19.825 9.4875 19.825 9.0375C19.825 8.55 19.45 8.175 19 8.175Z"
            clip-rule="evenodd"/>
        </svg>
      </button>
    </div>
          ...
          <ul class="mb-6 flex flex-col gap-1.5">
            <li v-for="(item, key) in menu_info?.menu" :key="key">
              <router-link :to="''"
                ...
                :class="{'bg-graydark text-white dark:bg-meta-4': sideBarPage.sharedString === item.name}">
                <font-awesome-icon :icon="item.icon" :style="{ width: '30px' }" /> {{ item.name }}
                <svg v-if="item.path === '#'" class="absolute right-4 top-1/2 -translate-y-1/2 fill-current" :class="{ 'rotate-180': sideBarPage.sharedString === item.name }"
              ...
              <div v-if="item.path === '#'" class="translate transform overflow-hidden" v-show="sideBarPage.sharedString === item.name">
                <ul class="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                  <li v-for="(item1, keys) in menu_info?.submenu[item.id]" :key="keys">
                    <router-link :to="''" class="group relative flex items-center gap-2.5 rounded-md px-4 my-2 font-medium text-gray-500 duration-300 ease-in-out hover:text-gray-400"
                      :class="{ '!text-white': item1.name === sidebarStore.selected }" @click="subMenuClick(item.name, item1.name, item1.path, item1.tab)">
                      <font-awesome-icon :icon="['far', 'circle']" /> {{ item1.name }}
                    </router-link>
                  </li>
                </ul>
              </div>
              <div class="translate transform overflow-hidden" v-show="sideBarPage.sharedString === item.name"></div>
            </li>
          </ul>
        </div>
      </nav>
