...
const props = defineProps<{
  isModalOpen: boolean;
  selected_donasi: any; // bisa berupa number (id) atau string (invoice)
}>();
...
const DetailKonfirmasi = ref<DetailKonfirmasi>();

// Fungsi untuk mengecek apakah selected_donasi adalah ID (number) atau Invoice (string)
const isId = (value: any): boolean => {
  return typeof value === 'number';
};

const fetchDetail = async () => {
  console.log('selected_donasi (REAL):', props.selected_donasi, typeof props.selected_donasi);

  try {
    let response;

    if (isId(props.selected_donasi)) {
      console.log('🟠 Panggil konfirmasi_detail');
      response = await konfirmasi_detail({ program_donasi_id: props.selected_donasi });
    } else {
      console.log('🟢 Panggil riwayat_detail');
      response = await riwayat_detail({ invoice: props.selected_donasi });
    }

    DetailKonfirmasi.value = response;
  } catch (error: any) {
    emit('status', { error: true, error_msg: 'Gagal memuat detail konfirmasi.' });
  }
};
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      fetchInfaqBanks();
      setTimeout(() => fetchDetail(), 100); // kasih delay 100ms biar props update dulu
    }
  },
);
...
const handleConfirmPayment = async () => {
  ...
  try {
    await update_konfirmasi({
      invoice: DetailKonfirmasi.value?.invoice,
    });
        ...
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-gray-800">
              {{
                DetailKonfirmasi?.status === 'success'
                  ? 'Detail Pembayaran Donasi'
          ...
          <div class="space-y-6">
            <div class="bg-gray-50 p-4 rounded-lg text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-800">{{
                  DetailKonfirmasi?.createdAt
                    ? new Date(DetailKonfirmasi?.createdAt).toLocaleString()
                    : '-'
                }}</span>
              </div>
              ...
              <div class="flex justify-between mt-1">
                ...
                <span
                  ...
                  :class="{
                    'bg-yellow-100 text-yellow-600': DetailKonfirmasi?.status === 'process',
                    'bg-green-100 text-green-600': DetailKonfirmasi?.status === 'success',
                    'bg-red-100 text-red-600': DetailKonfirmasi?.status === 'failed',
                  }"
                >
                  {{ DetailKonfirmasi?.status }}
                </span>
              </div>
            </div>

            <!-- Tampilan untuk status SUCCESS -->
            <div v-if="DetailKonfirmasi?.status === 'success'" class="text-center space-y-4">
            ...
            <div
              v-else-if="DetailKonfirmasi?.konfirmasi_pembayaran === 'sudah_dikirim'"
            ...
            <div class="pt-2">
              <div
                v-if="
                  DetailKonfirmasi?.status === 'success' ||
                  DetailKonfirmasi?.konfirmasi_pembayaran === 'sudah_dikirim'
                "
