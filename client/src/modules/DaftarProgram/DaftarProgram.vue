...
// Service API
import { get_daftar_program, edit_daftar_program, delete_daftar_program } from '@/service/daftar_program'
...
<template>
  ...
  <table v-else class="table-fixed w-full border-collapse bg-white text-sm">
    ...
    <FormAdd
      ...
      @status="(payload) => displayNotification(payload.error_msg || 'Program Berhasil Ditambahkan.', payload.error ? 'error' : 'success')"
    />
