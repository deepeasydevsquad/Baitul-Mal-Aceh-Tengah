...
const asnafId = 5; // ID untuk Asnaf Fisabilillah
...
const handlePrint = () => {
  ...
  document.title = `Laporan Penyaluran Asnaf Fisabilillah ${tahun === '0' ? 'Semua Tahun' : 'Per Tahun ' + tahun}`;
...
<template>
  ...
  <div v-else class="min-h-screen p-4 print:p-0 print:m-0">
    <div
      class="print-area font-sans flex flex-col flex flex-col flex flex-col flex flex-col flex flex-col"
      style="color: black; font-size: 8pt; line-height: 1.3; background: white"
    >
      <!-- Header -->
      <div class="flex justify-between items-start mb-3">
        <div>
          <h1 class="text-lg font-bold mb-1">Laporan Penyaluran Asnaf Fisabilillah</h1>
        ...
        <tbody>
          ...
          <tr v-else>
            <td colspan="7" class="border border-black px-2 py-3 text-center text-gray-700">
              Laporan Penyaluran Asnaf Fisabilillah Tidak Ditemukan
            </td>
          </tr>
        </tbody>
      </table>
