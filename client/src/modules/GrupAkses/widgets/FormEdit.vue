...
import { list_menu, edit_grup, list_menu_by_id } from '@/service/grup_akses'
  ...
  try {
    const payload = menus.value
      ...
      .map((menu) => ({
        ...
        path: menu.path,
        icon: menu.icon,
        Submenus: (menu.Submenus || []).filter((s) => selectedSubmenus.value.includes(s.id)),
      }))

    const body = {
      id: props.id,
      name: formName.value,
      group_access: JSON.stringify(payload), // <-- kirim string
    }

    const response = await edit_grup(body)
...
watch(
  ...
  async (newId) => {
    ...
    try {
      ...
      // pastikan menu sudah ke-load
      if (!menus.value.length) {
        await fetch_menu()
      }

      const response = await list_menu_by_id({ id: newId }) // cukup kirim id langsung kalau backend terima
      formName.value = response.name

      const groupAccess =
        typeof response.group_access === 'string'
          ? JSON.parse(response.group_access || '[]')
          : response.group_access || []

      selectedMenus.value = []
      selectedSubmenus.value = []

      groupAccess.forEach((menu: any) => {
        selectedMenus.value.push(menu.id)
        if (menu.Submenus?.length) {
          menu.Submenus.forEach((sub: any) => {
            selectedSubmenus.value.push(sub.id)
          })
        }
      })
    } catch (err) {
      console.error('Gagal ambil detail grup:', err)
    }
  },
  { immediate: true },
)
